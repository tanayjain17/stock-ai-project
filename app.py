...
def yf_safe(ticker, period="1y"):
    ...
    # ✅ Fix MultiIndex from Yahoo
    if isinstance(df.columns, pd.MultiIndex):
        df.columns = df.columns.get_level_values(0)

    # ✅ Continuous calendar for smooth chart
    df = df.asfreq("D")
...
ticker = st.text_input(
    "Enter Ticker (RELIANCE.NS, GOLDBEES.NS, GC=F)",
    "RELIANCE.NS"
)
...
SCAN_LIST = [
    ...
    "TCS.NS","ITC.NS","GOLDBEES.NS","SILVERBEES.NS"
]
...
if st.button("Run Scan"):
    for s in SCAN_LIST:
        ...
        st.markdown(
            f"<span style='color:{color}'><b>{s}</b> → {verdict}</span>",
            unsafe_allow_html=True
        )
